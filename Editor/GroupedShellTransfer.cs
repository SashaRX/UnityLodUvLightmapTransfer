...
namespace LightmapUvTool
{
    public static class GroupedShellTransfer
    {
        ...
        public static TransferResult Transfer(Mesh targetMesh, Mesh sourceMesh)
        {
            ...
            for (int tsi = 0; tsi < tgtShells.Count; tsi++)
            {
                ...
                else
                {
                    ...
                    // Cross-source UV2 overlap guard for interpolation (same as xform).
                    // Although interp stays within source UV2 convex hull, tightly packed
                    // UV2 regions can still produce triangle-level overlaps at boundaries.
                    if (uv2_interp.Count > 0)
                    {
                        Vector2 interpBMin = new Vector2(float.MaxValue, float.MaxValue);
                        Vector2 interpBMax = new Vector2(float.MinValue, float.MinValue);
                        foreach (var kv in uv2_interp)
                        {
                            interpBMin = Vector2.Min(interpBMin, kv.Value);
                            interpBMax = Vector2.Max(interpBMax, kv.Value);
                        }
                        foreach (var region in placedShellUv2Regions)
                        {
                            if (region.srcShell >= 0 && region.srcShell == chosenSrc) continue;
                            if (interpBMin.x < region.max.x && interpBMax.x > region.min.x &&
                                interpBMin.y < region.max.y && interpBMax.y > region.min.y)
                            {
                                issuesInterp += uv2_interp.Count; // heavy penalty
                                break;
                            }
                        }
                    }
            ...
            // ── Post-transfer overlap repair ──
            // Detect actual triangle-level UV2 overlaps between shells from different
            // source shells, and fix them by re-projecting affected vertices inward.
            RepairUv2Overlaps(result.uv2, tgtTris, tgtShells, result.faceToTargetShell,
                result.targetShellToSourceShell, result.vertexToSourceShell,
                srcShells, srcUv0, srcUv2, srcTris,
                triUv0A, triUv0B, triUv0C,
                triUv2A, triUv2B, triUv2C,
                triNormal, tUv0, tVerts, tNormals,
                shellUv0Bvh, srcUv2Min, srcUv2Max);
        ...
        // ═══════════════════════════════════════════════════════════
        //  Post-transfer UV2 overlap repair
        //  Detects triangle-level UV2 overlaps between shells from
        //  different source shells and repairs by re-projecting
        //  affected vertices strictly within their source UV2 region
        //  with a small inward margin.
        // ═══════════════════════════════════════════════════════════

        static void RepairUv2Overlaps(
            Vector2[] uv2, int[] tgtTris,
            List<UvShell> tgtShells,
            int[] faceToTargetShell,
            int[] targetShellToSourceShell,
            int[] vertexToSourceShell,
            List<UvShell> srcShells,
            Vector2[] srcUv0, Vector2[] srcUv2, int[] srcTris,
            Vector2[] triUv0A, Vector2[] triUv0B, Vector2[] triUv0C,
            Vector2[] triUv2A, Vector2[] triUv2B, Vector2[] triUv2C,
            Vector3[] triNormal,
            Vector2[] tUv0, Vector3[] tVerts, Vector3[] tNormals,
            TriangleBvh2D[] shellUv0Bvh,
            Vector2[] srcUv2Min, Vector2[] srcUv2Max)
        {
            int faceCount = tgtTris.Length / 3;
            if (faceCount < 2) return;

            // Extract UV2 shells for overlap detection
            var uv2Shells = UvShellExtractor.Extract(uv2, tgtTris);
            if (uv2Shells.Count < 2) return;

            // Map UV2 shells to source shells via dominant face → target shell → source shell
            var uv2ShellToSrc = new int[uv2Shells.Count];
            for (int si = 0; si < uv2Shells.Count; si++)
            {
                var tgtShellCounts = new Dictionary<int, int>();
                foreach (int f in uv2Shells[si].faceIndices)
                {
                    if (f < faceToTargetShell.Length)
                    {
                        int ts = faceToTargetShell[f];
                        if (ts >= 0 && ts < targetShellToSourceShell.Length)
                        {
                            int src = targetShellToSourceShell[ts];
                            if (src >= 0)
                            {
                                tgtShellCounts.TryGetValue(src, out int cnt);
                                tgtShellCounts[src] = cnt + 1;
                            }
                        }
                    }
                }
                int bestSrc = -1, bestCnt = 0;
                foreach (var kv in tgtShellCounts)
                    if (kv.Value > bestCnt) { bestSrc = kv.Key; bestCnt = kv.Value; }
                uv2ShellToSrc[si] = bestSrc;
            }

            // Find overlapping face pairs (diff-src only) using SAT test
            var overlapFaces = new HashSet<int>(); // faces involved in diff-src overlaps
            var overlapFaceToSrc = new Dictionary<int, int>(); // face → its source shell

            for (int i = 0; i < uv2Shells.Count; i++)
            {
                for (int j = i + 1; j < uv2Shells.Count; j++)
                {
                    int srcI = uv2ShellToSrc[i], srcJ = uv2ShellToSrc[j];
                    if (srcI < 0 || srcJ < 0 || srcI == srcJ) continue; // skip same-src

                    // AABB pre-filter
                    if (uv2Shells[i].boundsMin.x >= uv2Shells[j].boundsMax.x ||
                        uv2Shells[i].boundsMax.x <= uv2Shells[j].boundsMin.x ||
                        uv2Shells[i].boundsMin.y >= uv2Shells[j].boundsMax.y ||
                        uv2Shells[i].boundsMax.y <= uv2Shells[j].boundsMin.y)
                        continue;

                    var facesI = uv2Shells[i].faceIndices;
                    var facesJ = uv2Shells[j].faceIndices;
                    if ((long)facesI.Count * facesJ.Count > 500000) continue;

                    for (int fi = 0; fi < facesI.Count; fi++)
                    {
                        int fI = facesI[fi];
                        int ai0 = tgtTris[fI*3], ai1 = tgtTris[fI*3+1], ai2 = tgtTris[fI*3+2];
                        ...
                        {
                            int fJ = facesJ[fj];
                            int bj0 = tgtTris[fJ*3], bj1 = tgtTris[fJ*3+1], bj2 = tgtTris[fJ*3+2];
                            ...
                            if (TrianglesOverlap2D(a0,a1,a2, b0,b1,b2))
                            {
                                overlapFaces.Add(fI);
                                overlapFaces.Add(fJ);
                                if (!overlapFaceToSrc.ContainsKey(fI))
                                    overlapFaceToSrc[fI] = srcI;
                                if (!overlapFaceToSrc.ContainsKey(fJ))
                                    overlapFaceToSrc[fJ] = srcJ;
                            }
                        }
                    }
                }
            ...
            if (overlapFaces.Count == 0) return;

            // Group overlapping faces by their UV2 shell
            var shellOverlapFaces = new Dictionary<int, List<int>>(); // uv2ShellIdx → faces
            for (int si = 0; si < uv2Shells.Count; si++)
            {
                foreach (int f in uv2Shells[si].faceIndices)
                {
                    if (overlapFaces.Contains(f))
                    {
                        if (!shellOverlapFaces.TryGetValue(si, out var list))
                        {
                            list = new List<int>();
                            shellOverlapFaces[si] = list;
                        }
                        list.Add(f);
                    }
                }
            ...
            // Strategy 1: Re-project overlapping vertices using source-constrained UV0 lookup
            // For each overlapping face, try to find a better UV2 from its assigned source
            // shell that doesn't overlap with the neighbor.
            int repairedByReproject = 0;
            var vertexReprojected = new HashSet<int>();

            foreach (var kv in shellOverlapFaces)
            {
                int uv2ShellIdx = kv.Key;
                var faces = kv.Value;
                int srcShellIdx = uv2ShellToSrc[uv2ShellIdx];
                if (srcShellIdx < 0 || srcShellIdx >= srcShells.Count) continue;

                // Compute UV2 centroid of this shell (for inward shrink fallback)
                var shell = uv2Shells[uv2ShellIdx];
                Vector2 shellUv2Centroid = Vector2.zero;
                int centCount = 0;
                foreach (int vi in shell.vertexIndices)
                {
                    if (vi < uv2.Length)
                    {
                        shellUv2Centroid += uv2[vi];
                        centCount++;
                    }
                }
                if (centCount > 0) shellUv2Centroid /= centCount;

                // Source UV2 bounds with margin for re-projection clamping
                const float kInwardMargin = 0.002f;
                Vector2 srcMin = srcUv2Min[srcShellIdx] + new Vector2(kInwardMargin, kInwardMargin);
                Vector2 srcMax = srcUv2Max[srcShellIdx] - new Vector2(kInwardMargin, kInwardMargin);

                foreach (int f in faces)
                {
                    for (int j = 0; j < 3; j++)
                    {
                        int vi = tgtTris[f * 3 + j];
                        if (vertexReprojected.Contains(vi)) continue;
                        if (vi >= uv2.Length || vi >= tUv0.Length) continue;

                        Vector2 currentUv2 = uv2[vi];

                        // Try re-projection from source shell's UV0 BVH
                        var srcBvh = (srcShellIdx < shellUv0Bvh.Length) ? shellUv0Bvh[srcShellIdx] : null;
                        Vector2 newUv2 = currentUv2;
                        bool found = false;
                        ...
                        if (srcBvh != null)
                        {
                            ...
                            TriangleBvh2D.HitResult2D hit;
                            if (tNrm.sqrMagnitude > 0.5f)
                                hit = srcBvh.FindNearestNormalFiltered(tUv0[vi], tNrm, triNormal, 0.0f);
                            else
                                hit = srcBvh.FindNearest(tUv0[vi]);

                            if (hit.faceIndex >= 0)
                            {
                                newUv2 = triUv2A[hit.faceIndex] * hit.u
                                       + triUv2B[hit.faceIndex] * hit.v
                                       + triUv2C[hit.faceIndex] * hit.w;
                                found = true;
                            }
                        ...
                        if (found)
                        {
                            // Clamp to source UV2 bounds with inward margin
                            newUv2.x = Mathf.Clamp(newUv2.x, srcMin.x, srcMax.x);
                            newUv2.y = Mathf.Clamp(newUv2.y, srcMin.y, srcMax.y);

                            // Apply small inward bias toward shell centroid
                            // to create separation from neighboring shells
                            const float kShrinkFactor = 0.02f;
                            newUv2 = Vector2.Lerp(newUv2, shellUv2Centroid, kShrinkFactor);

                            uv2[vi] = newUv2;
                            vertexReprojected.Add(vi);
                            repairedByReproject++;
                        }
                        else
                        {
                            // Fallback: small inward shrink toward shell centroid
                            const float kFallbackShrink = 0.03f;
                            uv2[vi] = Vector2.Lerp(currentUv2, shellUv2Centroid, kFallbackShrink);
                            vertexReprojected.Add(vi);
                            repairedByReproject++;
                        }
                    }
            ...
            if (repairedByReproject > 0)
                UvtLog.Info($"[GroupedTransfer] Overlap repair: {repairedByReproject} vertices " +
                    $"re-projected ({overlapFaces.Count} overlapping faces in " +
                    $"{shellOverlapFaces.Count} shells)");
        }

        // SAT helpers (same algorithm as TransferValidator)
