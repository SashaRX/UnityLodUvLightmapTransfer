...
namespace LightmapUvTool
{
    public static class GroupedShellTransfer
    {
        ...
        public static TransferResult Transfer(Mesh targetMesh, Mesh sourceMesh)
        {
            ...
            // ── Post-transfer: per-triangle repair for inverted UV2 ──
            // Per-vertex UV0 projection can map vertices of one target triangle to
            // different source triangles → UV2 from incompatible atlas regions → inverted winding.
            // Fix: search ALL source tris across all matched shells, try multiple candidates,
            // project onto source plane, pick best that gives positive winding.
            // Multiple passes handle cascading effects from shared vertices.
            {
                int tgtFaceCount = tgtTris.Length / 3;
                int repaired = 0;

                // Collect all source faces from all matched shells
                var allSrcFaces = new HashSet<int>();
                for (int si = 0; si < srcShells.Count; si++)
                    foreach (int sf in srcShells[si].faceIndices)
                        allSrcFaces.Add(sf);
                var srcFaceList = new List<int>(allSrcFaces);

                // Pre-compute source centroids
                var srcCent3D = new Vector3[srcTriCount];
                for (int sf = 0; sf < srcTriCount; sf++)
                    srcCent3D[sf] = (triPosA[sf] + triPosB[sf] + triPosC[sf]) / 3f;

                for (int pass = 0; pass < 4; pass++)
                {
                    int passFixed = 0;

                    for (int f = 0; f < tgtFaceCount; f++)
                    {
                        ...
                        float sa = SignedArea2D(result.uv2[i0], result.uv2[i1], result.uv2[i2]);
                        if (sa >= 0) continue; // positive winding, OK

                        if (i0 >= tVerts.Length || i1 >= tVerts.Length || i2 >= tVerts.Length) continue;
                        Vector3 cent = (tVerts[i0] + tVerts[i1] + tVerts[i2]) / 3f;
                        Vector3[] tP = { tVerts[i0], tVerts[i1], tVerts[i2] };

                        // Find top-K closest source tris by centroid distance
                        const int K = 8;
                        var topK = new List<(int sf, float dist)>(K + 1);
                        for (int si = 0; si < srcFaceList.Count; si++)
                        {
                            int sf = srcFaceList[si];
                            float d = (srcCent3D[sf] - cent).sqrMagnitude;
                            if (topK.Count < K || d < topK[topK.Count - 1].dist)
                            {
                                topK.Add((sf, d));
                                topK.Sort((a, b) => a.dist.CompareTo(b.dist));
                                if (topK.Count > K) topK.RemoveAt(K);
                            }
                        }

                        // Try each candidate: project target verts onto source tri plane
                        Vector2[] bestUv = null;
                        float bestScore = float.MaxValue;

                        for (int ki = 0; ki < topK.Count; ki++)
                        {
                            int sf = topK[ki].sf;
                            Vector3 sA = triPosA[sf], sB = triPosB[sf], sC = triPosC[sf];
                            Vector3 e0 = sB - sA, e1 = sC - sA;
                            Vector3 sN = Vector3.Cross(e0, e1);
                            float area2 = sN.magnitude;
                            if (area2 < 1e-10f) continue;
                            sN /= area2; // unit normal
                            ...
                            var cUv = new Vector2[3];
                            float maxExtrap = 0;
                            for (int j = 0; j < 3; j++)
                            {
                                // Project target vertex onto source triangle plane
                                Vector3 v = tP[j] - sA;
                                float distToPlane = Vector3.Dot(v, sN);
                                Vector3 proj = tP[j] - distToPlane * sN - sA;

                                float bv = (d11 * Vector3.Dot(proj, e0) - d01 * Vector3.Dot(proj, e1)) * invD;
                                float bw = (d00 * Vector3.Dot(proj, e1) - d01 * Vector3.Dot(proj, e0)) * invD;
                                float bu = 1f - bv - bw;
                                cUv[j] = triUv2A[sf] * bu + triUv2B[sf] * bv + triUv2C[sf] * bw;

                                // Extrapolation penalty
                                float ext = Mathf.Max(0, Mathf.Max(-bu, Mathf.Max(-bv, -bw)));
                                if (ext > maxExtrap) maxExtrap = ext;
                            }

                            float cSa = SignedArea2D(cUv[0], cUv[1], cUv[2]);
                            if (cSa <= 0) continue; // still inverted

                            // Score: extrapolation penalty + distance
                            float score = maxExtrap * 100f + topK[ki].dist;
                            if (score < bestScore)
                            {
                                bestScore = score;
                                bestUv = cUv;
                        ...
                        if (bestUv == null) continue;

                        result.uv2[i0] = bestUv[0];
                        result.uv2[i1] = bestUv[1];
                        result.uv2[i2] = bestUv[2];
                        passFixed++;
                    }

                    repaired += passFixed;
                    if (passFixed == 0) break; // converged
                }

                if (repaired > 0)
                    UvtLog.Info($"[GroupedTransfer] '{targetMesh.name}': repaired {repaired} inverted tri(s) via single-source re-projection");
            }
